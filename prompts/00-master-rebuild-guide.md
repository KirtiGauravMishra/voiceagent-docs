...
- **Design explicitly for Redis being briefly unavailable**, since the whole real-time architecture leans on it: if Redis can't be reached when a new call is about to start, fail that one call gracefully (fallback TwiML apologizing + hangup, or route to the Gather-mode pipeline if that path doesn't need Redis) rather than crashing the handler. If Redis drops mid-call, the in-flight session loses its externally-recoverable state — detect this (a failed read/write to the session key) and end that call cleanly (farewell message + hangup) rather than leaving it hung with no further turns. Log these as alertable errors; don't let a Redis blip silently degrade into dead air on a live call.
...
**Webhook idempotency — every side effect, not just the headline one**: Twilio (and any other webhook-firing provider — Cal.com, etc.) retries delivery automatically if it doesn't get a fast 200 response, so every webhook handler **will** receive duplicate deliveries of the same event in normal operation, not just as a failure edge case. It's not enough to guard the one "big" side effect (e.g. firing a `call_completed` trigger exactly once) — check *every* side effect a handler performs (creating a `Notification`, incrementing a counter, appending a transcript entry, sending an outbound webhook) against current record state before acting, so a retried delivery is a safe no-op end to end, not just at the trigger layer.

### Data integrity: deleting an entity other records still reference
`Agent`, `Contact`, and `PhoneNumber` are referenced by historical records (`Call.agentId`, `Call.contactId`, `Campaign.phoneNumberId`, a `Workflow` node's `agentId`, etc.) that must remain meaningful after the referenced entity is gone — a deleted agent shouldn't turn every past call's detail page into a broken "agent: undefined" reference. Two complementary rules:
- **Denormalize a display-name snapshot at the time of reference** (e.g. `Call.agentName` set once when the call is created) so historical UI always has something sensible to show, independent of whether the source record still exists.
- **Block deletion where it would break something *active***, not just historical: reject deleting an `Agent` that's still assigned to an `active` campaign, an active workflow's call node, or a phone number's `inboundAgentId`, with a clear error naming what's still using it — don't silently cascade-delete or silently leave a dangling reference in active configuration. Deleting an agent with only historical (non-active) references is fine.
...
- **Every list/table page needs a designed empty state** — "no agents yet, create your first one" with a call-to-action button, not just a blank table or a bare "No data" string. This applies to every list view across the app (agents, calls, campaigns, leads, contacts, workflows, knowledge base, voices, notifications) — build the empty state into the shared list-page pattern from the start rather than adding it page-by-page later.
- **Never render user-supplied or API-returned text as raw HTML.** Call transcripts, agent system prompts, contact/lead names and notes, knowledge-base filenames, and workflow labels all ultimately come from user input or a third-party transcription/LLM response — render them as plain text content (React's default JSX text-node escaping handles this correctly on its own) and never pipe any of it through `dangerouslySetInnerHTML` or an equivalent raw-HTML sink. If you ever need limited rich text (e.g. a formatted note), sanitize with an allowlist-based library — don't trust the string as-is.
- **Accessibility is a build requirement, not a polish pass**: every shared primitive (`Modal`, `Select`, dropdown menus) must be keyboard-operable — modals trap focus while open and restore it to the triggering element on close, dropdowns open/close/navigate via keyboard, every icon-only button has an `aria-label`, and focus states are visible (don't remove the default outline without replacing it with an equally visible one). Build this into the shared primitives once in step 10 of the build order below — every page that reuses them inherits it for free, rather than needing an accessibility pass per page later.
